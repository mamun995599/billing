...
function isProcessRunning() {
    ...
    // Check if the process is running (Android/Termux compatible)
    exec("ps -p $pid", $output, $retval);
    return $retval === 0 && count($output) > 1;
}
...
function startBackupProcess() {
    ...
    // Start the process in the background (Termux compatible)
...
function stopBackupProcess() {
    ...
    // Terminate the process (Termux compatible)
    exec("kill -9 $pid", $output, $retval);
